#include <inttypes.h>
#include <string>

#include "inc_data_table.h"

struct Struct
{
  int64_t cIntKey;
  std::string cStrKey;
  int64_t cValue;
};

MOMO_DATA_COLUMN_STRUCT(Struct, cIntKey);
MOMO_DATA_COLUMN_STRUCT(Struct, cStrKey);
MOMO_DATA_COLUMN_STRUCT(Struct, cValue);

typedef momo::DataColumnList<momo::DataColumnTraits<Struct>> DataColumns;
typedef momo::DataTable<DataColumns> DataTable;

#define SETUP_INT DataTable table(DataColumns(cIntKey, cValue)); auto ind = table.AddUniqueHashIndex(cIntKey)
#define RESERVE_INT(size) table.Reserve(size)
#define INSERT_INT(key, value) table.AddRow(cIntKey = key, cValue = value)
//#define DELETE_INT(key) table.RemoveRow(table.FindByUniqueHash(ind, cIntKey == key)->GetNumber(), false);
#define FIND_INT_EXISTING(key) if (table.FindByUniqueHash(ind, cIntKey == key).GetCount() != 1) { printf("error"); exit(1); }
#define FIND_INT_MISSING(key) if (table.FindByUniqueHash(ind, cIntKey == key).GetCount() != 0) { printf("error"); exit(2); }
#define CHECK_INT_VALUES(value) for (auto rowRef : table) if (rowRef[cValue] != value) { printf("error"); exit(3); }

#define SETUP_STR DataTable str_table(DataColumns(cStrKey, cValue)); auto str_ind = str_table.AddUniqueHashIndex(cStrKey)
#define RESERVE_STR(size) str_table.Reserve(size)
#define INSERT_STR(key, value) str_table.AddRow(cStrKey = key, cValue = value)
//#define DELETE_STR(key) str_table.RemoveRow(str_table.FindByUniqueHash(str_ind, cStrKey == key)->GetNumber(), false);
#define FIND_STR_EXISTING(key) if (str_table.FindByUniqueHash(str_ind, cStrKey == key).GetCount() != 1) { printf("error"); exit(4); }
#define FIND_STR_MISSING(key) if (str_table.FindByUniqueHash(str_ind, cStrKey == key).GetCount() != 0) { printf("error"); exit(5); }

#include "template.c"

